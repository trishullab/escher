...
class Cub2011(Dataset):
    ...
    def __init__(
        ...
        if with_attributes:
            assert os.path.exists(
                root + f"/cub/attributes_{attributes_version}.txt"
            ), print(
                f"No attributes found, please run description.py for attributes_{attributes_version} first!"
            )
            with open(root + f"/cub/attributes_{attributes_version}.txt") as file:
                self.attributes = file.read().strip().split("\n")
            file.close()

            # Add clip prediction as the prior
            with open(root + "/cub/clip_classification.txt") as file:
                clip_classification = [eval(line) for line in file.read().split("\n")]
            file.close()

            with open("./data/cub/classes.txt", "r") as file:
                classes = [
                    cla.split(".")[1].replace("_", " ")
                    for cla in file.read().strip().split("\n")
                ]
            file.close()

            self.attributes = [
                f"{classes[pred - 1]} with {attr}"
                for attr, pred in zip(self.attributes, clip_classification)
            ]
    ...
    def _load_metadata(self):
        images = pd.read_csv(
            os.path.join(self.root, "cub", "images.txt"),
            sep=" ",
            names=["img_id", "filepath"],
        )
        image_class_labels = pd.read_csv(
            os.path.join(self.root, "cub", "image_class_labels.txt"),
            sep=" ",
            names=["img_id", "target"],
        )
        train_test_split = pd.read_csv(
            os.path.join(self.root, "cub", "train_test_split.txt"),
            sep=" ",
            names=["img_id", "is_training_img"],
        )
        ...
        if self.mode == "train":
            ...
            if self.with_attributes:
                self.attributes = np.array(self.attributes)[
                    self.data.is_training_img == 0
                ]
            self.data = self.data[self.data.is_training_img == 0]

        if self.with_attributes:
            self.attributes = np.array(self.attributes)[
                self.data.target <= self.num_classes
            ]
    ...
    def __init__(self, root, preprocess=None):
        ...
        for idx, cla in enumerate(classnames):
            if tuple(cla.split(", ")) in cls2foldername:
                if os.path.exists(
                    os.path.join(root, cls2foldername[tuple(cla.split(", "))])
                ):
                    for file in os.listdir(
                        ...
                        self.files.append(
                            os.path.join(
                                root, cls2foldername[tuple(cla.split(", "))], file
                            )
                        )
...
class WaterBirds(Dataset):
    ...
    def __init__(
        ...
        train_remove_indices = np.where(
            (self.group_array[train_indices] == 1)
            | (self.group_array[train_indices] == 2)
        )
        ...
        test_remove_indices = np.where(
            (self.group_array[test_indices] == 0)
            | (self.group_array[test_indices] == 3)
        )
...
class CatDog(Dataset):
    ...
    def __init__(
        ...
        if with_attributes:
            assert os.path.exists(
                root + f"/PetImages/attributes_{attributes_version}.txt"
            ), print(
                f"No attributes found, please run description.py for attributes_{attributes_version} first!"
            )
    ...
    def _load_metadata(self):
        images = pd.read_csv(
            os.path.join(self.root, "PetImages", "images.txt"),
            sep=" ",
            names=["img_id", "filepath"],
        )
        image_class_labels = pd.read_csv(
            os.path.join(self.root, "PetImages", "image_class_labels.txt"),
            sep=" ",
            names=["img_id", "target"],
        )
        train_test_split = pd.read_csv(
            os.path.join(self.root, "PetImages", "train_test_split.txt"),
            sep=" ",
            names=["img_id", "is_training_img"],
        )
